...
describe('Bitrix24 canonical analytics reader', () => {
  it('paginates tracked website deals and maps only analytics fields', async () => {
    ...
    expect(calls[0]!.body).toMatchObject({
      ...
      select: expect.arrayContaining(['UF_CRM_1780724113']),
      start: 0,
    });
  ...
  it.each([
    [{ CREATED_TIME: '' }, 'empty'],
    [{}, 'missing'],
    [{ CREATED_TIME: 'not-a-timestamp' }, 'unparseable'],
  ])('fails closed on %s stage-history timestamps', async (timestampFields, _label) => {
    const client = createBitrixAnalyticsClient({
      webhookUrl: 'https://example.bitrix24.ru/rest/1/key/',
      portalId: 'member-1',
      fetchImpl: (async () => new Response(JSON.stringify({ result: { items: [{
        ID: '1', CATEGORY_ID: '0', STAGE_ID: 'NEW', ...timestampFields,
      }] } }), { status: 200 })) as typeof fetch,
    });
    await expect(client.getStageHistory('42')).rejects.toThrow(/invalid CREATED_TIME/);
  });
  ...
  it('fails closed on a malformed source deal identity', async () => {
    const client = createBitrixAnalyticsClient({
      webhookUrl: 'https://example.bitrix24.ru/rest/1/key/',
      portalId: 'member-1',
      fetchImpl: (async () => new Response(JSON.stringify({
        result: [{ ID: '42', UF_CRM_1780724113: 'not-an-id' }],
      }), { status: 200 })) as typeof fetch,
    });
    await expect(client.listTrackedDeals()).rejects.toThrow(/invalid source deal ID/);
  });

  it('normalizes equivalent offset timestamps to canonical UTC', async () => {
    const client = createBitrixAnalyticsClient({
      webhookUrl: 'https://example.bitrix24.ru/rest/1/key/',
      portalId: 'member-1',
      fetchImpl: (async () => new Response(JSON.stringify({ result: [{
        ID: '42', UF_CRM_1780724113: '42',
        DATE_CREATE: '2026-09-01T12:00:00+03:00',
        DATE_MODIFY: '2026-09-01T12:00:00.250+03:00',
      }] }), { status: 200 })) as typeof fetch,
    });
    ...
    await expect(client.listTrackedDeals()).resolves.toEqual([
      expect.objectContaining({
        ...
        modifiedAt: '2026-09-01T09:00:00.250Z',
      }),
    ]);
  });

  it.each([
    [{ DATE_MODIFY: '2026-09-01T10:00:00Z' }, 'DATE_CREATE'],
    [{ DATE_CREATE: '', DATE_MODIFY: '2026-09-01T10:00:00Z' }, 'DATE_CREATE'],
    [{ DATE_CREATE: 'not-a-timestamp', DATE_MODIFY: '2026-09-01T10:00:00Z' }, 'DATE_CREATE'],
    [{ DATE_CREATE: '2026-09-01', DATE_MODIFY: '2026-09-01T10:00:00Z' }, 'DATE_CREATE'],
    [{ DATE_CREATE: '2026-02-30T09:00:00Z', DATE_MODIFY: '2026-09-01T10:00:00Z' }, 'DATE_CREATE'],
    [{ DATE_CREATE: '2026-09-01T09:00:00Z' }, 'DATE_MODIFY'],
    [{ DATE_CREATE: '2026-09-01T09:00:00Z', DATE_MODIFY: '' }, 'DATE_MODIFY'],
    [{ DATE_CREATE: '2026-09-01T09:00:00Z', DATE_MODIFY: 'not-a-timestamp' }, 'DATE_MODIFY'],
  ])('fails closed on malformed or missing deal timestamps: %s', async (timestamps, field) => {
    const client = createBitrixAnalyticsClient({
      webhookUrl: 'https://example.bitrix24.ru/rest/1/key/',
      portalId: 'member-1',
      fetchImpl: (async () => new Response(JSON.stringify({ result: [{
    ...
    await expect(client.listTrackedDeals()).rejects.toThrow(new RegExp(`invalid ${field}`));
  });
});
